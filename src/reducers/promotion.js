...
const promotions = (state = initialState, action) => {
    switch (action?.type) {
        ...
        case SEARCH_PROMOTION:
          return {
            ...state,
            searchPromotion: action?.payload,
          };
            ...
            allPromotions: state?.allPromotions?.map((promotion) => {
              if (promotion?.id === action?.payload.id) {
                return {
                  ...promotion,
                  selectedCampaigns:
                    promotion?.selectedCampaigns && promotion?.selectedCampaigns?.length > 0
                      ? promotion?.selectedCampaigns?.push(action?.payload.selectedCampaigns)
            ...
            allPromotions: state?.allPromotions?.map((promotion) => {
              if (promotion?.id === action?.payload.id) {
                return {
                  ...promotion,
                  selectedSurveys:
                    promotion?.selectedSurveys && promotion?.selectedSurveys?.length > 0
                      ? promotion?.selectedSurveys?.push(action?.payload.selectedSurveys)
